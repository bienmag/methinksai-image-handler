...
function DicomItem({ item }) {
  return (
    <Link className="hover:bg-my2" to={`/image/${item.id}`}>
      <div className=" text-left px-4 grid border-my2 grid-flow-col border-b-[1px] min-h-12 w-full py-2 mt-2 items-center">
        <div className="flex flex-col  ">
          <h1 className="text-md text"> Example dicom image</h1>
      ...
      {images.length === 0 ? (
        ...
        <div className="flex flex-col text-center">
          ...
          <div className="flex flex-col">
            ...
            {dicoms.map((item) => (
              <DicomItem key={item.id} item={item}></DicomItem>
            ))}
          </div>
          <div className="py-8">{images.length + dicoms.length} files</div>
        </div>
      )}
    </div>
  );
